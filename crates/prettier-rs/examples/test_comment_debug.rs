...
fn main() {
    ...
    let r = format_str(input, Format::Yaml, &opts).expect("format failed");
    println!("Input:    {input:?}");
    println!("Output:   {r:?}");
    ...
    let input2 = "a:\n  #123";
    let r2 = format_str(input2, Format::Yaml, &opts).expect("format failed");
    println!("\nInput:    {input2:?}");
    println!("Output:   {r2:?}");
    println!("Expected: {:?}", "a:\n  #123\n");
    ...
    let input3 = "foo: bar\n\n# End Comment - Don't remove previous empty line\n";
    let r3 = format_str(input3, Format::Yaml, &opts).expect("format failed");
    println!("\nInput:    {input3:?}");
    println!("Output:   {r3:?}");
    println!(
        "Expected: {:?}",
    ...
    let input4 = "before:\n\n  # before.comment\nafter:\n  # after.comment\n";
    let r4 = format_str(input4, Format::Yaml, &opts).expect("format failed");
    println!("\nInput:    {input4:?}");
    println!("Output:   {r4:?}");
    println!(
        "Expected: {:?}",
    ...
    // sequence.yml
    let input_s = "-  - a\n\n   # - b\n\n   # - c\n\n   - e\n";
    let rs = format_str(input_s, Format::Yaml, &opts).expect("format failed");
    println!("\n=== sequence.yml ===");
    println!("Output:");
    ...
    let r5 = format_str(input5, Format::Yaml, &opts).expect("format failed");
    println!("\nInput:    {input5:?}");
    println!("Output:   {r5:?}");
